...
1. After the single shared pre-write lookup, treat a deterministic operation such as changing an existing material color to red as a direct edit. Do not start external research, build a full Brief, rebuild the shader, or require a render loop when the requested change is already clear.
...
Before the first scene write in the request, satisfy the shared [knowledge-and-memory.md](../houdini-visual-research/references/knowledge-and-memory.md) contract. If the main task or visual-research Skill already completed that lookup, reuse it for all later material writes and do not run a LookDev-specific duplicate. For substantive LookDev, also read [build-brief-and-review.md](../houdini-visual-research/references/build-brief-and-review.md), contribute only the material, lighting, risk, output, and acceptance decisions needed by the compact Brief, and use external research when the visual-research contract requires it.
...
3. Confirm current availability before authoring. Reuse the required local-help lookup, and when exact installed types remain uncertain, query `hia_search_node_types` and `hia_node_help` narrowly and serially. Treat `mtlxstandard_surface` as the intended MaterialX standard-surface concept, not permission to hard-code a versioned internal type name.
