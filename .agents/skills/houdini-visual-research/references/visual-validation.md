...
4. For a Full visual task, identify only meaningful visible milestones. Normally consider completed primary structure, completed materials or lighting when in scope, and final handoff, but merge adjacent stages and skip unchanged or inapplicable ones. Use one representative frame for an ordinary static claim. For a complete 3D primary-form or delivery claim, use the smallest view set that reveals identity and silhouette, major negative spaces, support/contact, and depth relationships; do not assume one view settles all of them. For animation, simulation, or a time-varying material or effect, choose a risk-proportionate short sequence or representative frame set that covers meaningful change, contacts or transitions, continuity, and any suspect or failed frame. Do not substitute one still for a dynamic completion claim or force a long flipbook merely for process.
5. Before judging a capture, confirm that the tool response contains actual image content, the intended camera and aspect were used, the actual returned frame or bounded sequence covers what was requested, and the original frame and view state were restored. A path, hash, metadata record, or `quality_status=passed` without image content is not visual observation; mark the claim visually unverified. Treat `quality_status` as capture-integrity and basic pixel-sanity evidence only, never proof of composition, subject visibility, or asset quality. Treat a low-resolution preview as bounded review evidence rather than the requested final output; if frame coverage or restoration is unproven, mark the claim unverified. Then inspect `quality_reasons`, `quality_metrics`, `display_match`, and `hdr_display_mismatch_risk`. If capture integrity is failed or warning, or display matching is `unverified`, recapture only when a bounded setting change can fix the evidence; otherwise request human inspection or mark the visual claim unverified.
...
7. For a project-team Full complex visual milestone, follow the Panel project contract in [full-goal-blueprint.md](full-goal-blueprint.md). After each stage, start the independent read-only Visual Review and Technical Review project Threads in parallel. Give unchanged visual claims and usable actual image content to Visual Review, and unchanged technical or semantic claims plus routed live evidence to Technical Review. Supervisor waits for both returns before accepting the stage or selecting a repair. Sol Ultra internal subagents may add non-overlapping review inside either Thread, but never replace the five project Threads or the two-review contract. Execution remains the sole writer; model or internal-subagent availability never changes the evidence bar. Use `$houdini-artifact-review` inside the applicable review Thread and compare in this order:
...
9. Name the largest visible or technical mismatch, let Supervisor approve the smallest set of controls or coherent region that addresses that one area, and let only Execution apply the fix. After a major or blocking fix, reacquire the affected live evidence and actual image content, then start both review Threads again in parallel with the same acceptance claims. Continue this Supervisor-controlled evidence/rework loop without a fixed iteration count until both applicable evidence sets are verified, the user stops, or a genuine blocker is evidenced. Reviewers never write the HIP or start their own loop. The bounded EffectSpec candidate-round limit below controls comparison cost only; it never turns an unverified stage into a pass.
