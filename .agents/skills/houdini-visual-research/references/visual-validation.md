...
5. Before judging a capture, confirm the intended camera and aspect were used, the actual returned frame or bounded sequence covers what was requested, and the original frame and view state were restored. Inspect the actual returned image content together with the result's objective `source_state`, exact frame lock, capture success, cook evidence, and temporal coverage. Treat a low-resolution preview as bounded review evidence rather than the requested final output; if the image is unavailable or frame, view, cook, or restoration evidence is unproven, mark the claim unverified. Codex judges composition, visibility, clipping, exposure, reference match, and other subjective qualities from the image; the runtime does not score them.
...
1. Before candidate execution, Codex keeps a temporary task-specific **EffectSpec** only in the current task context: goal, target and mutable scope, a few visual or temporal objectives, only relevant metrics and controls, fixed frame and view samples, and observable success conditions.
2. Establish one low-cost baseline, capture it under fixed comparison conditions, then evaluate two or three bounded candidates one at a time. For each candidate, use one explicit `hia_execute_hom` call for only that bounded change, followed by explicit capture and claim-specific validation. Inspect the real scene after a failed write; return to or adjust the baseline only through another deliberate bounded HOM call after the current state is known. HIA does not automatically rank candidates, restore a baseline, clear caches, assemble contact sheets, or create the EffectSpec or Evaluation. Retrieved knowledge may inform control direction, version evidence, failure causes, and comparable cases, but it does not prewrite this task's scoring criteria.
3. Codex evaluates the actual previews for every specified frame and view together with the relevant returned Houdini data. Record an explicit ranking and best candidate, each candidate's main issue, the single largest remaining issue, and the small set of controls allowed to change next. Without usable preview evidence and relevant Houdini data, do not declare a best candidate or completion.
...
Intermediate failures are not report triggers by themselves. Preserve the original error, inspect the active Houdini version/build and relevant local help, then make a bounded in-scope compatibility, syntax, parameter, or workflow correction and revalidate. Before another `hia_execute_hom` write, read `errors[*].partial_scene_changes_possible` and `scene_change_status`. A syntax or undo-group setup failure reported as unchanged may be corrected directly because execution did not begin. A possible, changed, or unknown scene state requires targeted inspection or `hia_scene_diff` first. HIA does not automatically undo a failed batch, and `unknown`, `partial`, or `NO_OBSERVED_EFFECT` validation does not prove completion. Do not retry the same failing action indefinitely or hide actual partial scene changes.
...
Use the actual path returned by `hia_capture_viewport`, always under the project-local `.runtime/cache/screenshots` directory. Store other temporary visual-check previews under `.runtime/cache/previews`, temporary research artifacts under `.runtime/cache`, and user-provided attachments under `.runtime/attachments`. Do not stage or commit these runtime artifacts.
