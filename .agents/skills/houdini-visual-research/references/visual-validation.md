...
7. For a Full complex visual milestone, give usable previews and relevant technical or semantic evidence to read-only `$houdini-artifact-review`. When native subagents are available, use multiple non-overlapping read-only reviewers covering at least identity/primary form and assembly/technical integrity, adding material, lighting, animation, simulation, network, or performance review only when in scope; the Director remains the sole writer. When subagents are unavailable, the Director performs the same domain-separated review as distinct read-only passes before the next write; reviewer availability changes independence, never the evidence bar. Compare in this order:
...
9. Name the largest visible mismatch and let only the main task change the smallest set of controls that addresses that one area. After a major or blocking fix, recapture actual image content and return the same acceptance claim to the applicable read-only reviewers. Continue this Director-controlled write-review loop until the requested bar is supported, the user stops, or a genuine blocker is evidenced. Reviewers never write the HIP or start their own loop.
