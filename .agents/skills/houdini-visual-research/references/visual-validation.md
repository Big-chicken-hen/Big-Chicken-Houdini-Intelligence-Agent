...
1. Before candidate execution, Codex keeps a temporary task-specific **EffectSpec** in the current task context or existing Goal state: goal, target and mutable scope, a few visual or temporal objectives, only relevant metrics and controls, fixed frame and view samples, and observable success conditions.
