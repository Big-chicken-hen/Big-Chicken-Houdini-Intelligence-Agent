...
4. Retrieval mode is explicit. `lexical` uses FTS5 without requiring Qwen. If the caller selected `hybrid` or `vector`, an unavailable or signature-mismatched Qwen encoder is an explicit failure; never substitute lexical results. The encoder only supplies retrieval vectors. Codex remains responsible for interpretation, research, planning, scene decisions, visual review, and memory wording.
