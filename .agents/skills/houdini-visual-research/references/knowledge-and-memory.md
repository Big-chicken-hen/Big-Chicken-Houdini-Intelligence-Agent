...
2. Do not issue another local-help search when another skill activates, planning begins, or execution starts. Resolve a remaining exact node or API uncertainty with the appropriate installed help, and use SideFX/original-source web research only when the unresolved question can materially change the decision. A materially changed user scope is a new request, not a reason to repeat the same lookup silently.
...
5. A bundled workflow card is a planning summary, not proof of current node behavior. When its evidence is incomplete, conflicting, unfamiliar, or version-sensitive in a way that can change the decision, use proportionate external research through current SideFX documentation and original sources as described in `research-and-sources.md`. Otherwise continue with installed help, live inspection, and real-scene validation.

A Context Pack is optional evidence and Direct tasks skip it. An unavailable or empty Context Pack or local lookup is missing evidence, not a terminal Goal state or restart reason. Do not repeat the same lookup; continue with available installed help, live inspection, and any necessary SideFX or original-source research, then mark only the unresolved claim unverified.
...
- Keep Supervisor responsible for deciding and explicitly authorizing `record`, `supersede`, and `delete`. Because Bridge exposes HIA tools only to Execution in a project-team Full Goal, Execution may transmit only that approved memory action. Planning, reviewers, and internal subtasks may return concise evidence or a memory candidate but do not write competing entries.
