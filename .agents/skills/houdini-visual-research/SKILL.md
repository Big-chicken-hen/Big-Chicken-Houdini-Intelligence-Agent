...
description: Research-led planning, procedural construction, and visual validation for recognizable, editable Houdini assets and complex modeling, FX, materials, animation, reference-image, ShaderToy/GLSL, Solaris, and Karma tasks. Use when the user asks to create a named asset or effect such as a vending machine, wooden cabin, or rain window even without saying "high quality"; cites an image, artist, paper, tutorial, website, or specific effect; or needs external technical research or visual diagnosis. Do not use for routine repository/plugin development, a simple successful Box or one-parameter read, or an ordinary HOM error that does not require visual or unfamiliar technical judgment.
---
...
2. Treat a request to generate a recognizable named asset or effect, such as a vending machine, wooden cabin, or rain window, as a complete, editable deliverable by default. Do not require the user to add "high quality"; only an explicit blockout or prototype request lowers the completion bar.
3. Treat other modeling, FX, look development, animation, lighting, reference matching, or final-image work with a meaningful visual result as complex. Choose semantic research depth (`none`, `light`, or `deep`) from [research-and-sources.md](references/research-and-sources.md), without fixed call counts or gates.
