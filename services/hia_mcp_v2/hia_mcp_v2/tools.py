...
TOOL_SPECS = (
    ToolSpec(
        ...
        "discovery",
        "Discover the HIA Houdini capability matrix by task or domain. Use this when unsure which high-signal tool to call. Batch related reads, but split complex asset writes into bounded semantic stages or coherent subsystems; review the real scene and images between authoring batches.",
    ...
    ToolSpec(
        ...
        "visual_feedback",
        "Capture the current visible viewport through Houdini's documented flipbook path only when visual verification is needed. Omitted dimensions derive from the live viewport; one supplied dimension preserves its aspect, and two supplied dimensions are honored exactly. A single frame remains the static default. For animation or simulation, use an explicit frames list or frame_range plus frame_step (at most 24 frames); the runtime locks, evaluates, and captures each requested frame, detects missing/failed/unchanged sequences, and records requested versus actual frames with bounded evidence. With return_image=true, a static capture returns its image content and a sequence returns bounded first/last image content; paths alone are not visual observation. validation_paths optionally force-cook critical nodes per frame. Restores the original camera/view, camera lock, and frame state, does not open MPlay or take focus. quality_status covers capture integrity and basic pixel sanity only, never composition, subject visibility, or asset quality; unverified OS HDR/display matching remains an explicit boundary. A safely saved current HIP uses its sibling .hia/screenshots directory; otherwise images fall back to HIA_CACHE_DIR/screenshots.",
        _object(
            {
