...
_PROJECT_PROTOCOL_INSTRUCTION = (
    ...
    " Use native web search when the action is to discover sources. Use the native"
    " openPage web action when the action is to read an explicit public URL. Never"
    " switch between them as a hidden fallback; report the selected action's failure."
