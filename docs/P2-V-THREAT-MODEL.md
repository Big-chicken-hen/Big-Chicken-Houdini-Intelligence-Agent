...
The production MCP registration admits only `houdini_scene_info` and `houdini_node_type_info`. `houdini_graph_validate`, `houdini_graph_apply`, and `houdini_graph_verify` are explicitly disabled and unregistered there; their use by the completed offline B3 harness or dormant B4A fake-HOM adapter grants no runtime capability. All other tool names and all unknown fields fail closed. Gate B4B, live `hou`, and every real write remain unauthorized.
...
- One successful apply is enclosed completely in one `hou.undos.group`. B4A verifies only the fake-HOM call boundary. Future B4B must prove that one manual Undo removes the selected request-owned graph and changes nothing else before scene writing can be called production-ready.
...
| An I/O, worker, or untrusted guard thread calls `hou` | Tampering, denial of service | The gateway's I/O side may only parse, validate, and enqueue immutable work. The injected control guard is not trusted to preserve scheduling: every `mutate`, `contain`, and `finalize` callback rechecks Houdini UI-main-thread identity at callback entry before adapter, scene, Undo, observer, or revision access. B4A wrong-thread failure performs zero fake-HOM mutation. Future live code may never fall back to the current thread, and fake evidence does not verify real HOM scheduling. |
...
| Fake callback behavior is mistaken for live-Houdini coverage | Tampering, repudiation, denial of service | B4A proves only that an event actually delivered during an armed expectation must carry the exact registered source identity; modeled root `ChildDeleted` uses the retained parent. A delivered mismatch, external, late, or off-main-thread event invalidates. Zero events are neither proof of no mutation nor proof that real HOM is observable. Observer installation on newly created nodes and event reliability for `createNode`, `setUserData`, parameters, connections, flags, and destruction remain fail-closed B4B capability-probe blockers. |
| Undo grouping omits a parameter, connection, flag, rollback, or post-exit proof | Tampering, repudiation | Enclose creation, typed parameters, wiring, flags, bounded postcondition, and commit bookkeeping in one `hou.undos.group`. Explicitly contain factory/enter/exit failures under the same authority lock. A missing candidate output uses the unified rollback and one Undo-close path. After successful Undo exit, repeat the full observed-state proof before publishing success; after rollback exit, repeat root/child path absence and baseline parent/fingerprint proof. Any exception after enter is attempted is indeterminate. B4A proves only this fake boundary; real Undo remains for B4B. |
...
B4A must prove that table and structurally different stairs traverse one dormant adapter path without an asset-role field, fixed structure, name-derived semantics, or per-node handler. It must also prove preflight zero-mutation failures, exact transform-to-`xform` resolution, typed parameter calls, exactly one fake Undo group, one revision advance, observed-state postconditions, all seven failure stages, exact-root rollback, interruption re-raise, indeterminate write latch, and preserved pre-existing content. Every result must exclude tokens, environment data, raw chat, tracebacks, and user paths. Frozen Schema hashes and the exact production two-tool `tools/list` must remain unchanged.

Future B4B real Houdini acceptance, which is not authorized by B4A, is limited to one selected fixture in one blank unsaved disposable HIP and must separately prove:
...
4. One successful request creates only one approved `/obj/HIA_Graph_<id>` root with no connections and exactly its normalized declared children, typed values, bidirectionally verified transaction-owned wiring/cardinality, flags, and graph digest.
5. One manual Undo removes the whole new graph and restores every pre-existing path, parameter, connection, flag, selection, and current-node value to the baseline.
...
At the time of this document, none of these real-`hou` write, cook, callback, rollback, version-compatibility, or Undo claims has been executed. B3 and B4A fake evidence do not change that status.
...
- Correct main-thread scheduling, node-type names, parameter semantics, callback delivery/source identities, observer attachment to newly created nodes, `setUserData` event coverage, cooking, rollback, and Undo behavior depend on the live Houdini build. Each is a separately approved B4B capability/acceptance blocker; B4A fake evidence cannot satisfy it.
...
- Production-readiness claims before separately approved B4B live-`hou` main-thread, post-verification, and one-manual-Undo evidence passes; fake-HOM rollback tests are not live rollback evidence.
