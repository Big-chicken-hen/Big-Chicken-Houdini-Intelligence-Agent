...
Gate B4A completed at commit `0b3b5a51c0ad95aee382794d74852d2d541bf880`. Gate B4B now authorizes one dedicated local acceptance Panel to inject the live `hou` module into that existing writer for the frozen stairs fixture, exactly one Apply, and one user-triggered Undo in a blank unsaved disposable HIP. The normal Panel, Bridge, app-server, MCP registration, launcher, and configuration remain unchanged; `houdini_graph_validate`, `houdini_graph_apply`, and `houdini_graph_verify` stay disabled and unregistered in the production Codex profile.
...
Those five names describe the frozen graph contract. Gate B3 exercised all five only inside an explicitly constructed offline fake harness; Gate B4A consumed the already validated graph contract only through an explicitly constructed dormant adapter and fake-HOM. Gate B4B calls that writer only from the dedicated one-shot local acceptance Panel and does not register a graph tool. The production runtime surface remains the strict two-tool subset `houdini_scene_info` and `houdini_node_type_info`.
...
Gate B3 records one pure-Python simulated Undo transaction, and Gate B4A verifies one fake-HOM Undo-group call boundary plus injected thread/clock guards. The current B4B user-run sequence must separately prove that one live apply and one manual Undo affect only the selected request-owned graph; neither B3 nor B4A proves real `hou.undos.group`, HOM main-thread scheduling, or live timing semantics.
...
13. B2C resolves only the exact project-local two-tool read-only MCP TOML and finite read chain. B3 proved pure-Python approval, transaction, rollback, postcondition, and simulated Undo behavior; B4A proved only the dormant adapter's fake-HOM call order and guards. The authorized B4B Panel is the sole local path for the finite live Apply/manual-Undo acceptance; its live cook, callback, rollback, `hou.undos.group`, and mutation behavior remain unresolved until the user completes that run.
...
### B4B — One-shot local live graph acceptance

The authorized entry is `hia_b4b_stairs_acceptance.pypanel`, not a production MCP route. Its controller reads only `tests/fixtures/p2_v/stairs_graph.json`, validates its frozen digest, builds the exact existing `SceneQueue` approval/claim/binding, and displays the complete baseline, normalized graph, side-effect summary, request digest, and approval-binding digest. The B1 fake attestation is used only as the existing queue's local ledger envelope; live authority comes from the strict injected Houdini read adapter and is rechecked immediately before mutation.

The Panel consumes a process-global latch before the single Apply call. Closing or reopening the pane cannot reset it. Every real HOM call remains on the captured UI main thread, inside the writer's one Undo group, and must produce the reviewed strict callback evidence or fail closed. Newly returned nodes receive verified observers before their next mutation. No additional write is made merely to probe event behavior.

After Apply, the controller independently checks the exact root, child types/names, typed parameters, connections in both directions, flags, graph digest, errors, unchanged baseline selection/current node, no extra content, and one revision advance. If the exact root remains, it stops in `WAIT_MANUAL_UNDO` with `manual_undo_required=true` whether the acceptance result is `ok=true` or strict evidence/ledger completion made it `ok=false`. The user must press Ctrl+Z once. The read-only verifier requires exact root deletion evidence, absence of every declared path, restored baseline `/obj` identity/fingerprint, selection/current node, and new unsaved HIP state. Only the `ok=true` apply followed by a complete verified restoration may close the Gate; cleanup restoration after `ok=false` remains a failed acceptance. No automatic Undo, Redo, retry, second fixture, or failure injection is permitted. A successful run still does not prove the writer's real rollback-through-`destroy()` path.
